...
function refreshAuthUI() {
  ...
  document
    ...
    .forEach((btn) => btn.classList.toggle("is-locked", !authed));
...
function generateOrderId() {
  return "NM-" + new Date().getFullYear() + "-" + String(Date.now()).slice(-6);
}
...
function kontoShowSection(id, el) {
  document
    .querySelectorAll(".konto-section")
    .forEach((s) => s.classList.remove("active"));
  document
    .querySelectorAll(".konto-nav__item")
...
function initKonto() {
  ...
  if (location.hash === "#orders") {
    const navOrders = document.querySelector(
      '.konto-nav__item[data-sec="orders"]',
    );

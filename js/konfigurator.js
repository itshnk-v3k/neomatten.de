...
const BRANDS = ['Audi', 'BMW', 'Mercedes', 'VW', 'Toyota', 'Hyundai', 'KIA', 'Skoda', 'Porsche', 'Ford'];
const MODELS = {
  Audi:     [['A4 (B9)', '2015–2023'], ['A6 (C8)', '2018–2024'], ['A7 (4G7)', '2010–2017'], ['Q5 (FY)', '2017–2024']],
  BMW:      [['3er (G20)', '2018–2024'], ['5er (G30)', '2017–2023'], ['X3 (G01)', '2017–2024']],
  Mercedes: [['C-Klasse (W205)', '2014–2021'], ['E-Klasse (W213)', '2016–2023'], ['GLC (X253)', '2015–2022']],
  VW:       [['Golf VII', '2012–2019'], ['Golf VIII', '2019–2024'], ['Passat (B8)', '2014–2023'], ['Tiguan II', '2016–2024']],
  Toyota:   [['Camry XV70', '2017–2024'], ['Corolla E210', '2018–2024'], ['RAV4 V', '2018–2024']],
  Hyundai:  [['Tucson III', '2015–2020'], ['Tucson IV', '2020–2024'], ['Santa Fe', '2018–2024']],
  KIA:      [['Sportage IV', '2015–2021'], ['Ceed III', '2018–2024'], ['Sorento IV', '2020–2024']],
  Skoda:    [['Octavia III', '2012–2020'], ['Octavia IV', '2019–2024'], ['Kodiaq', '2016–2024']],
...
function renderBrands(filter = '') {
  ...
  wrap.innerHTML = list.map(b => {
    ...
    return `
    <button class="brand-row ${selectedBrand === b ? 'active' : ''}" onclick="selectBrand('${b}')">
...
function renderModels() {
  ...
  if (!selectedBrand) { wrap.innerHTML = ''; return; }
  wrap.innerHTML = MODELS[selectedBrand].map(([name, years]) => `
...
// Подсветка зон на SVG-схеме салона авто
function highlightZones(setType) {
  const zones = { fl: false, fr: false, rl: false, rr: false };
  ...
  if (setType === 'premium') { zones.fl = true; zones.fr = true; zones.rl = true; zones.rr = true; }
  Object.keys(zones).forEach(z => {
    const el = document.getElementById('zone-' + z);
    if (el) { el.style.fill = zones[z] ? '#8B1A1A' : '#E8E8E8'; el.style.opacity = zones[z] ? '0.85' : '1'; }
  });
}
